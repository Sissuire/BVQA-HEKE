...
class DatasetValidation_with_Average(Dataset):
    ...
    def __init__(self, dataset='LIVE', basep='/mnt/disk/yongxu_liu/datasets', nsnippet=12):
    ...
    def get_test_batch(self, file, width, height, frms):
        ...
        for i in range(T):
            ...
            for j in range(N):
                ...
                img = self.AveragePooling(img, scale) / 255.
                img = transform(img)
    ...
    def __getitem__(self, index):
        ...
        dis = self.get_test_batch(file, width, height, frms)
...
class HEKE_BVQA_r2p1d(nn.Module):
    ...
    def forward(self, x):
        ...
        return x.flatten(1), (torch.cat([f1_ave, f1_max, f1_std], dim=1).cpu().numpy(),
...
def run_features(args):
    ...
    testdataset = DatasetValidation_with_Average(dataset=database, basep='/mnt/disk/yongxu_liu/datasets')
    ...
    with torch.no_grad():
        for i, (vid, mos) in enumerate(test_loader):
            ...
            vid = vid.view(torch.Size([batch * ns]) + sh2).to(device)
            y_label.append(mos.cpu().numpy())

            outputs, ff = model(vid)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Blindly assessing and feature extraction with HEKE-4')
    ...
    parser.add_argument('--model', default='r2p1d_HEKE_4', type=str,
