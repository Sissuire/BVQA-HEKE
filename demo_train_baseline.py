...
from network import HEKE_BVQA_r2p1d as myModel
from dataset import DatasetTraining, DatasetValidation
...
def train_model(model, device, optimizer, scheduler, train_loader, test_loaders, args, init_epoch=0):
    ...
    for epoch in range(init_epoch, init_epoch+num_epochs):
        ...
        if epoch_loss < best_epoch_loss:
            ...
            state = {'state_dict': model.module.state_dict(), 'optimizer': optimizer.state_dict(), 'epoch': epoch, 'best_epoch_loss': best_epoch_loss}
        ...
        for dataset in test_loaders.keys():
            ...
            gt_l = []
            for dis, mos in test_loaders[dataset]:
                batch = dis.shape[0]
                ns = dis.shape[1]
                sh2 = dis.shape[2:]
...
def run(opt):
    ...
    try:
        ...
        opt.resume = True
        opt.load = './save_baseline_v3_1/model_at_000_epoch.pkl'
    except KeyError:
        print('No debug')
    ...
    basep = '/home/sissuire/datasets/WELL-Set/dis_png'   # PATH to WELL-Set
    info = 'dataset.clean_info.lthpc.pkl'                 # WELL-Set information file
    trainset = DatasetTraining(info=info, basep=basep, isDynamic=True,
...
def parse_args():
    parser = argparse.ArgumentParser(description='Multi-Knowledge Driven VQA\n '
                                                 'single group for the four weak labels')
    ...
    #                     help='enable faster training with variable size input')
